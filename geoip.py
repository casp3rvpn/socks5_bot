"""
GeoIP module - determines proxy country.
"""
import asyncio
import aiohttp
...
class GeoIP:
    """Determines country for IP addresses."""
    ...
    def get_flag(self, country_code: str) -> str:
        ...
        return COUNTRY_FLAGS.get(country_code.upper(), UNKNOWN_FLAG)
    
    async def lookup_ip(self, ip: str) -> Optional[str]:
        """
        Lookup country code for IP address.
        
        Returns:
            Country code (e.g., 'US', 'DE') or None if failed
        """
        # Check cache first
    ...
    async def lookup_proxy(self, proxy: Dict) -> Dict:
        """
        Lookup country for a proxy.
    ...
    async def lookup_multiple(self, proxies: list) -> list:
        """Lookup countries for multiple proxies."""
        tasks = [self.lookup_proxy(p) for p in proxies]
    ...
    def lookup_sync(self, proxies: list) -> list:
        """Synchronous wrapper."""
        return asyncio.run(self.lookup_multiple(proxies))
